...
from functools import partial
from timm.models import create_model
from LateralConv import *
from LearnableDiscreteTanh import *
...
#离散的tanh函数
class Spiking_lateral_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.conv = nn.Conv2d(self.num_heads,self.num_heads,1,1)
        self.conv2 = LateralConv(self.num_heads)
        self.bn = nn.BatchNorm2d(self.num_heads)
        self.linear = nn.Linear(self.num_heads,self.num_heads)

        self.ternary_tanh_excite = LearnableDiscreteTanh(init_alpha=2.0, init_threshold=0.3)
        self.ternary_tanh_inhibit = LearnableDiscreteTanh(init_alpha=4.0, init_threshold=0.7)
        #self.ternary_tanh = TernaryTanhWithGradient.apply
    ...
    def forward(self, x):
        ...
        attention_map = k.transpose(-2, -1) @ v  #t b head c c
        attention_map2 = attention_map.flatten(0,1)
        excite_map = attention_map2
        # excite_map = self.conv(attention_map2)
        # excite_map = self.bn(excite_map)
        # inhibit_map = self.conv(attention_map2)
        # inhibit_map = self.bn(inhibit_map)
        #inhibit_map = self.conv(attention_map2)
        #inhibit_map = self.bn(inhibit_map)

        inhibit_map = self.conv2(attention_map2)
        # 添加 tanh 归一化，限制范围到 [-1, 1]
        excite_map = torch.tanh(excite_map / (excite_map.abs().max() + 1e-8))
        inhibit_map = torch.tanh(inhibit_map / (inhibit_map.abs().max() + 1e-8))

        a = self.ternary_tanh_excite(excite_map)
        b = self.ternary_tanh_inhibit(inhibit_map)

        #a = torch.sigmoid(excite_map) #excited_rate
        #b = torch.tanh(inhibit_map)  #inhibit_rate
        commen_map  = a + b
        commen_map = commen_map.reshape(T,B,self.num_heads,C // self.num_heads,C // self.num_heads)
        attention_lateral_map = attention_map + commen_map

        x = (q @ attention_lateral_map) * self.scale
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.tssa = Spiking_lateral_Attention(dim)
        #self.tssa = Spiking_Self_Attention(dim)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.ssa = Spiking_lateral_Attention(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop)
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        #patch_embed1 = coding('ADD')

        stage1 = nn.ModuleList([TokenSpikingTransformer(
            dim=embed_dims // 2,  mlp_ratio=mlp_ratios, qkv_bias=qkv_bias,
        ...
        stage2 = nn.ModuleList([SpikingTransformer(
            dim=embed_dims, mlp_ratio=mlp_ratios, qkv_bias=qkv_bias,
            qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
            norm_layer=norm_layer, sr_ratio=sr_ratios)
            for j in range(1)])
...
@register_model
def LIASFormer(pretrained=False, **kwargs):
    model = vit_snn(
        patch_size=16, embed_dims=256, mlp_ratios=1,
