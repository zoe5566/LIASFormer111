...
from spikingjelly.datasets import es_imagenet
from spikingjelly.datasets import cifar10_dvs
from EnhancedLateralInhibition import *

__all__ = ['LIASFormer_10_768']
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.tssa = Spiking_lateral_Attention(dim, num_heads)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads):
        super().__init__()
        self.attn = Spiking_lateral_Attention(dim, num_heads=num_heads)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop)
...
class hierarchical_spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2],pretrained=None,**kwargs
                 ):
        super().__init__()
...
def LIASFormer_10_384(T=1, **kwargs):
...
def LIASFormer_10_512(T=1, **kwargs):
    model = hierarchical_spiking_transformer(
        T=T,
        img_size_h=224, img_size_w=224,
...
@register_model
def LIASFormer_10_768(**kwargs):
    model = hierarchical_spiking_transformer(
        #T=T,
        img_size_h=224, img_size_w=224,
        patch_size=16, embed_dims=768, num_heads=12, mlp_ratios=4,
        in_channels=2, num_classes=1000, qkv_bias=False,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=10, sr_ratios=1,
        **kwargs
    )
    return model
